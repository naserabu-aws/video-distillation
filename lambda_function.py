...
def start_yamnet_classification(bucket_name, key, timestamp, sanitized_filename):
    ...
    using the raw video file directly
    """
    ...
    # Create the transform job with proper content type for video files
    response = boto3.client('sagemaker').create_transform_job(
        ...
        TransformInput={
            ...
            'ContentType': 'application/octet-stream',  # Raw video/audio file
...
def lambda_handler(event, context):
    try:
        ...
        try:
            ...
            # Start YAMNet classification with the raw video file
            # The SageMaker model now handles all preprocessing internally
            try:
